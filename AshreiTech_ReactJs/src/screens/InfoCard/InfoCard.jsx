        ...
        <ScrollTrigger onEnter={() => setCounter(true)} onExit={() => setCounter(false)}>
            {
                ...
                <div className={`   ${props?.data?.length === 4 ? 'px-sm-5 justify-content-between' : 'd-flex flex-wrap justify-content-center'} align-items-center pt-4`}>
                    {props?.data?.map(({ image, heading, discription }, index) => {
                        const updatedDescription = parseInt(discription.replace(/[K+]/g, ''), 10);
                        return (
                            <div key={index} className={`${item} ${props?.data?.length === 4 ? ' ' : 'col-6 col-md-3 col-lg-2 flex-column justify-content-center text-center' } d-flex  flex-lg-row align-items-center  mb-3  text-lg-left`}>
                                <div>
